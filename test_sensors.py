...
from sensors import GPS, IMU, WheelOdometry, SensorHealth, SensorFusionManager
...
class TestGPS:
    ...
    def test_gps_initialization(self):
        ...
        gps = GPS(noise_std=2.0, dropout_prob=0.1)
    ...
    def test_gps_measurement_accuracy(self):
        ...
        gps = GPS(noise_std=0.0, dropout_prob=0.0)  # No noise or dropouts
        ...
        np.testing.assert_allclose(measurement, true_position + gps.position_bias, atol=0.01)
        
    def test_gps_noise_characteristics(self):
        """Test GPS noise has correct statistical properties"""
        gps = GPS(noise_std=5.0, dropout_prob=0.0)
    ...
    def test_gps_dropout_behavior(self):
        ...
        gps = GPS(noise_std=0.0, dropout_prob=0.3)
        true_position = np.array([0.0, 0.0, 0.0])
    ...
    def test_gps_coordinate_transformations(self):
        ...
        gps = GPS(noise_std=0.0, dropout_prob=0.0)
...
class TestIMU:
    ...
    def test_imu_initialization(self):
        ...
        imu = IMU(accel_noise_std=0.1, gyro_noise_std=0.05)
    ...
    def test_imu_measurement_structure(self):
        ...
        imu = IMU(accel_noise_std=0.0, gyro_noise_std=0.0)
    ...
    def test_imu_bias_drift(self):
        ...
        imu = IMU(accel_noise_std=0.0, gyro_noise_std=0.0, bias_drift_rate=0.01)
    ...
    def test_imu_failure_modes(self):
        ...
        # Test dropout failure
        imu.simulate_failure('dropout')
        measurement = imu.get_measurement(np.zeros(3), np.zeros(3))
        # Should have high probability of None measurements
...
class TestWheelOdometry:
    ...
    def test_wheel_odometry_initialization(self):
        ...
        odometry = WheelOdometry(wheel_base=0.5, wheel_radius=0.1)
        assert odometry.wheel_base == 0.5
    ...
    def test_differential_drive_kinematics(self):
        ...
        odometry = WheelOdometry(wheel_base=1.0, wheel_radius=0.1, encoder_noise_std=0.0, slip_factor=0.0)
        ...
        assert abs(delta_pose['dx'] - expected_distance) < 1e-6
        assert abs(delta_pose['dy']) < 1e-6
        assert abs(delta_pose['dtheta']) < 1e-6
    ...
    def test_rotation_kinematics(self):
        ...
        assert abs(delta_pose['dx']) < 1e-6
        assert abs(delta_pose['dy']) < 1e-6
        assert abs(delta_pose['dtheta'] - expected_angle) < 1e-6
        
    def test_slip_modeling(self):
        """Test wheel slip reduces odometry accuracy"""
        odometry_no_slip = WheelOdometry(wheel_base=1.0, wheel_radius=0.1, slip_factor=0.0)
        odometry_with_slip = WheelOdometry(wheel_base=1.0, wheel_radius=0.1, slip_factor=0.2)
        ...
        assert delta_with_slip['dx'] < delta_no_slip['dx']
        
    def test_encoder_noise(self):
        """Test encoder noise affects measurements"""
        odometry = WheelOdometry(wheel_base=1.0, wheel_radius=0.1, encoder_noise_std=0.1)
        
        measurements = []
        for _ in range(100):
            delta_pose = odometry.compute_delta_pose(1.0, 1.0, 0.1)
            measurements.append(delta_pose['dx'])
...
class TestSensorFusionManager:
    ...
    def test_redundant_sensor_measurements(self):
        ...
        measurements = manager.get_all_measurements(position, acceleration, angular_velocity, wheel_speeds)
        
        assert 'gps' in measurements
        assert 'imu' in measurements
        assert 'odometry' in measurements
        assert len(measurements['gps']) <= 2  # May have dropouts
        assert len(measurements['imu']) <= 2  # May have failures
    ...
    def test_sensor_failure_simulation(self):
        ...
        position = np.array([100.0, 200.0, 50.0])
        measurements = manager.get_all_measurements(position, np.zeros(3), np.zeros(3), (0.0, 0.0))
        
        # Should have few or no GPS measurements during constellation failure
        assert len(measurements['gps']) < 2
        
    def test_sensor_health_monitoring(self):
        """Test manager tracks overall sensor health"""
        manager = SensorFusionManager()
        
        health_status = manager.get_sensor_health()
        
        assert 'gps' in health_status
        assert 'imu' in health_status
        assert 'odometry' in health_status
        assert len(health_status['gps']) == 2
        assert len(health_status['imu']) == 2
...
class TestSensorIntegration:
    ...
    def test_multiple_sensor_independence(self):
        ...
        gps1 = GPS(noise_std=1.0, dropout_prob=0.1, sensor_id=1)
        gps2 = GPS(noise_std=2.0, dropout_prob=0.2, sensor_id=2)
